...
function addPraise(nid,uid){
    let sql = `INSERT INTO praise(nid,uid) VALUES(?,?)`;
...
function cancelPraise(nid,uid){
    let sql = `DELETE FROM praise WHERE nid = ? AND uid = ?`;
    let sqlParams = [nid,uid];
    return new Promise((resolve,reject)=>{
        query(sql,sqlParams,function(err,res){
            if(err){
...
function checkIfPraise(nid,uid){
    let sql = `SELECT * FROM praise WHERE nid = ? AND uid = ?`;
    let sqlParams = [nid,uid];
    return new Promise((resolve,reject)=>{
        query(sql,sqlParams,function(err,res){
            if(err){
...
function findPraiseCountByNid(nid){
    let sql = `SELECT count(*) as praise FROM praise WHERE nid = ?`;
    let sqlParams = [nid];
    return new Promise((resolve,reject)=>{
        query(sql,sqlParams,function(err,res){
            if(err){
